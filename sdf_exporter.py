...
def export_sdf(prefix):
    ...
    # Moved: bpy.ops.wm.collada_export(filepath=prefix_path+dae_filename, selected=True, check_existing=False, filter_blender=False, filter_image=False, filter_movie=False, filter_python=False, filter_font=False, filter_sound=False, filter_text=False, filter_btx=False, filter_collada=True, filter_folder=True, filemode=8)
    ...
    for o in mesh_objects:
        ...
        # 1 model and 1 link
        model = ET.SubElement(sdf, "model", attrib={"name":o.name})        
        if not o.rigid_body.enabled: # static/dynamic object?
            static = ET.SubElement(model, "static")
            static.text = "true"
        link = ET.SubElement(model, "link", attrib={"name":o.name+"link"})
        
        sdf_filename = o.name + '.sdf'
        model_config_filename = 'model' + '.config' # .config must have name model.config
        model_name = o.name
        dae_filename = o.name + '.dae'

        prefix_path = prefix + o.name + "/"
        
        o.select_set(True)
        
        # Exports the dae file of selected and its associated textures
        bpy.ops.wm.collada_export(filepath=prefix_path+dae_filename, selected=True, check_existing=False, filter_blender=False, filter_image=False, filter_movie=False, filter_python=False, filter_font=False, filter_sound=False, filter_text=False, filter_btx=False, filter_collada=True, filter_folder=True, filemode=8)
...
if __name__ == "__main__":
    ...
    prefix_path = "/home/olin/Robotics/warehouse/" + str(now.month) +"-"+str(now.day) +"-"+ str(now.hour) +"-"+ str(now.minute) +"-"+ str(now.second) + "/"
